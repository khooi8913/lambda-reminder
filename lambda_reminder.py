...
# =============================================================================
# END OF LUNAR CALENDAR IMPLEMENTATION
# =============================================================================
...
# SEND EMAIL FUNCTION
# =============================================================================
def send_email(email_subject, email_string):
    gmail_user = 'SENDER@GMAIL.COM'
    gmail_app_password = os.environ['MAIL_PASS']
    
    sent_from = gmail_user
    sent_to = ['RECIPIENT_1@GMAIL.COM', 'RECIPIENT_2@GMAIL.COM']
    
    sent_subject = email_subject
    sent_body = (email_string)
...
# =============================================================================
# LUNAR CALENDAR CHECKER & EMAIL CONSTRUCTION
# =============================================================================
def check_lunar():
    ct = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
    ln = Lunar(ct)

    current_date = ln.localtime.date()
    current_time = ln.localtime.time()

    email_string += '今日 公历 {} 为 {} ({}) {}年\n'.format(ln.localtime.date(), ln.ln_date_str(), ln.gz_year(), ln.sx_year())
    email_subject = '温馨提醒 - 今天 是 ' + ln.ln_date_str()
    
    if '初一' in ln.ln_date_str() or '十五' in ln.ln_date_str():
        email_string += '\n'
        email_string += '提醒您，今天吃素哦～\n\n'
        email_string += 'It\'s vegetarian day!\n'
        email_string += '\n'
        email_string += '此电邮生成于' + str(current_date) + ' ' + str(current_time)
        send_email(email_subject, email_string)
    else:
        ct += datetime.timedelta(days=1)
        ln = Lunar(ct)
        if '初一' in ln.ln_date_str() or '十五' in ln.ln_date_str():
            email_string += '明天 公历 {} 为 {} ({}) {}年\n'.format(ln.localtime.date(), ln.ln_date_str(), ln.gz_year(), ln.sx_year())
            email_string += '\n'
            email_string += '提醒您，明天吃素哦～\n\n'
            email_string += 'Tomorrow will be vegetarian day!\n'
            email_string += '\n'
            email_string += '此电邮生成于' + str(current_date) + ' ' + str(current_time)
            send_email(email_subject, email_string)
# =============================================================================
# END OF LUNAR CALENDAR CHECKER & EMAIL CONSTRUCTION 
# =============================================================================
